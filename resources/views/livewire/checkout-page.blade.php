				...
				<div class="bg-white rounded-xl shadow p-4 sm:p-7 dark:bg-slate-900">
					...
					<div class="mb-6">
						...
						<div class="mt-2">
							...
							<input wire:model='first_name' readonly class="w-full rounded-lg border py-2 px-3 bg-gray-100 dark:bg-gray-700 dark:text-white dark:border-none @error('first_name') border-red-500 @enderror" id="name" type="text">
							@error('first_name')
						...
						<div class="mt-4">
							...
							<input wire:model='email' readonly class="w-full rounded-lg border py-2 px-3 bg-gray-100 dark:bg-gray-700 dark:text-white dark:border-none @error('email') border-red-500 @enderror" id="email" type="email">
							@error('email')
								<div class="text-red-500 text-sm">{{ $message }}</div>
							@enderror
						</div>
						...
						<div class="mt-4">
							...
							<input wire:model='telp' class="w-full rounded-lg border py-2 px-3 dark:bg-gray-700 dark:text-white dark:border-none @error('telp') border-red-500 @enderror" id="telp" type="text">
							@error('telp')
								<div class="text-red-500 text-sm">{{ $message }}</div>
							@enderror
						</div>
				...
				<div class="bg-white mt-4 rounded-xl shadow p-4 sm:p-7 dark:bg-slate-900">
					...
					<ul class="divide-y divide-gray-200 dark:divide-gray-700" role="list">
						@foreach ($cart_items as $ci)
    						<li class="py-3 sm:py-4" wire:key='{{ $ci['id'] ?? uniqid() }}'>
								<div class="flex items-center">
									<div class="flex-shrink-0">
										<img alt="{{ $ci['name'] ?? '' }}" class="w-12 h-12 rounded-full"
     										src="{{ isset($ci['image']) ? url('storage/'.$ci['image']) : asset('images/default-product.png') }}">
									</div>
