              ...
              @foreach ($orders as $order)
                @php
                    ...
                    if ($order->payment_method == 'cod'){
                      $payment_method = '<span class="bg-gray-600 py-1 px-3 rounded text-white shadow">Cash on Delivery</span>';
                    }
                    if ($order->payment_method == 'stripe'){
                      $payment_method = '<span class="bg-purple-600 py-1 px-3 rounded text-white shadow">Stripe</span>';
                    }
                @endphp
                ...
                wire:key='{{ $order->id}}'>
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">{!! $payment_method !!}</td>
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">{{ Number::currency($order->total, 'IDR') }}</td>
